...
- Original: Token deployed on Ethereum at nonce 46 to `0xb47e...BBB`
...
```solidity
Recovery constant recovery = Recovery(address(0));
string constant original_chain = "bsc";     // Chain where contract was originally deployed
string constant rescue_chain = "eth";       // Chain where funds are stuck
uint256 constant targetedNonce = 4;         // Original deployment nonce
address constant targetedContract = 0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56;
address constant broadcaster = 0xF07C30E4CD6cFff525791B4b601bD345bded7f47;
```
...
1. Update token list in `Deploy.s.sol`:
...
> [!WARNING]
> only append "--broadcast --private-key PRIVATE_KEY_GOES_HERE" to the command when executing the recovery
